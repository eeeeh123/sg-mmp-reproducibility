"""Launch Llama-3.2 validation jobs in the background with a clean env."""
...
def main() -> None:
    ...
    if not args:
        raise SystemExit("No experiments/fix_llama32/run.py arguments provided")
    out_path = LOG_DIR / f"{ns.name}.out.log"
    err_path = LOG_DIR / f"{ns.name}.err.log"
    out = open(out_path, "w", encoding="utf-8")
    err = open(err_path, "w", encoding="utf-8")
    cmd = [sys.executable, "experiments/fix_llama32/run.py", *args]
